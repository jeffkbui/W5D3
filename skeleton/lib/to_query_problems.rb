...
def price_like_shiny_mouse
  ...
  execute(<<-SQL) 
    ...
    WHERE
      toys1.name != 'Shiny Mouse'
    AND
      toys2.name = 'Shiny Mouse'
    ORDER BY
      toys1.name ASC
...
def toys_that_jet_owns
  ...
  execute(<<-SQL)
    SELECT
      cats.name, othertoys.name
    FROM
      cats
    JOIN
      cattoys AS cats1 ON cats1.cat_id = cats.id
    JOIN
      toys AS toys1 ON cats1.toy_id = cats1.id
    JOIN
      cattoys AS othercats ON othercats.cat_id = cats.id
    JOIN
      toys AS othertoys ON othercats.toy_id = othercats.id
    WHERE
      cats1.name = 'Jet' AND othercats.name != 'Jet'
    ORDER BY
      cats.name ASC
  SQL
end
